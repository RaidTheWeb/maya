...
int usexsdt(void) {
    return acpi_rsdp->rev >= 2 && acpi_rsdp->xsdtaddr;
}
...
void *acpi_findsdt(char sign[4], uint64_t idx) {
    ...
    for(uint64_t i = 0; i < entry; i++) {
        ...
        if(usexsdt()) sdt = (sdt_t *)(((uint64_t *)&acpi_rsdt->start)[i] + HIGHER_HALF);
        else sdt = (sdt_t *)(((uint32_t *)&acpi_rsdt->start)[i] + HIGHER_HALF);

        if(!memcmp(&sdt->sign, sign, 4)) {
            if(count != idx) {
                count++;
                continue;
            }
