...
void madt_init(void) {
    madt_t *madt = (madt_t *)acpi_findsdt("APIC", 0); // get Multiple APIC Descriptor Table (MADT)
    if(madt == NULL) {
    ...
    while(1) {
        if(madt->header.len - sizeof(madt_t) - off < 2) break;

        madtheader_t *header = (madtheader_t *)(madt->data + off);
        switch(header->id) {
            case 0:
